...
## Smooth default (product promos)

For **premium, non-snappy** scene changes (default for most promos):

- Prefer **`springTiming({ config: { damping: 20–26, mass: 0.85–1 }, durationInFrames: 24–32 })`** with **`fade()`** instead of very short **`linearTiming`** or sharp **slides**.
- **Longer overlaps** read smoother on screen than 10–12 frame linear crossfades.
- Reserve **fast / punchy** transitions for explicit briefs (e.g. user-selected fast-paced style).
